...
shared_ptr<Score> Score::getInstance(int x, int y, int w, int h, std::string txt1) {
...
int Score::tick(const std::vector<std::shared_ptr<Sprite>>& sprites, GameSession* gameSession) {
	return 0;
}
...
Score::Score(int x, int y, int w, int h, std::string txt) : Sprite(x, y, w, h, txt) {
	scoreText = txt;
