...
shared_ptr<Missile> Missile::getInstance(int x, int y, std::string image) {
...
int Missile::tick(const std::vector<std::shared_ptr<Sprite>>& sprites, GameSession* gamesession) {
...
Missile::Missile(int x, int y, std::string image) : Sprite(x, y, 5, 20, image) {
	texture = IMG_LoadTexture(sys.ren, image.c_str());
	missileHit = Mix_LoadWAV("invaderkilled.wav");
}

bool Missile::checkCollision(std::shared_ptr<Sprite> s) {
